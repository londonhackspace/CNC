...
#        to_new_xc=self.stat.position[0]-self.stat.g5x_offset[0] - self.stat.g92_offset[0] - self.stat.tool_offset[0] - xcres
    ...
    def on_tsh_released(self, gtkbutton, data = None):
        ...
        if self.ocode ("O<zdown> call") == -1:
            return
        a=self.probed_position_with_offsets()
        # Get the toolsetter offset and save it in #5169 (G28 W)
        # Adjust the height by the switch trip distance.
	tool_set_height = a[2] + 0.0040
        s="#5169 = %f" % tool_set_height
        if self.gcode(s) == -1:
            return
        self.lb_tsh_z.set_text( "%.4f" % float(tool_set_height) )
        self.add_history("Toolsetter Offset ","Z",0,0,0,0,0,0,0,0,tool_set_height,0,0)
